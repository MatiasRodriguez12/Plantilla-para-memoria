...
\begin{figure}[h]
	...
	\caption{Esquema de interconexión entre el ESP32 y los módulos externos.}
...
Para lograr una comunicación compatible y confiable entre el ESP32 y la red CAN del sistema, fue necesario incorporar un módulo transceptor provisto por Enertik, como se muestra en la figura \ref{fig:placa_transceiver}.
...
\begin{figure}[h]
	...
	\caption{Módulo transceptor comercial perteneciente a Enertik.}
	\label{fig:placa_transceiver}
\end{figure}

Basado en esta premisa, se decidió replicar y adaptar este módulo transceptor
...
La figura \ref{fig:im_hw_transceiver} muestra un diagrama completo sobre cómo fue la implementación de este módulo transceptor.
...
\begin{itemize}
    ...
    \item Componente 4: divisor resistivo. Corresponde al arreglo de resistencias encargado de adaptar los niveles de tensión y permitir una comunicación UART segura entre el ESP32 y el módulo transceptor.
    
    \item Componente 5: módulo transceptor. Se trata de una placa comercial provista por Enertik. En este prototipo se decidió incorporar un conector en el PCB experimental para facilitar su montaje y desmontaje de forma segura.
    
    \item Componente 6: módulo W5100. De manera similar al módulo transceptor, se utilizó un conector compatible que permite su montaje y desmontaje de forma sencilla y segura.
\end{itemize}
...
En el caso del módulo transceptor, dado que se conocía completamente su
...
\begin{itemize}
	...
	\item Componente 2: MCP2551. Forma parte del módulo transceptor, encargado de adaptar los niveles de tensión de la línea CAN.
	\item Componente 3: divisor resistivo. Corresponde al arreglo de resistencias encargado de adaptar los niveles de tensión y permitir una comunicación UART segura entre el ESP32 y el módulo transceptor.
...
A modo de resumen, la figura \ref{fig:diagrama_completo} muestra todos los componentes de hardware empleados en el diseño y como se relacionan unos con otros.

\begin{figure}[h]
	\centering
	\includegraphics[scale=0.39]{./Figures/diagrama_completo.png}
	\caption{Diagrama final de los componentes de hardware empleados.}
...
En este contexto, para el presente trabajo se implementaron seis tareas
...
Se optó por este tipo de estructura porque resulta adecuada para la recepción
...
Esta tarea se encarga de gestionar la comunicación
...
Para su funcionamiento, se utiliza un contador interno que cumple la función de
temporizador. En primer lugar, establece la comunicación Modbus TCP y se
...
Con este objetivo, se realizó una modificación en el firmware del módulo PRC10. Se incorporó una función que permite generar
...
validadas por la etapa de decodificación. A partir de estas,
...
Previo a la implementación, se definió un criterio de diseño orientado
a la organización modular de la interfaz. Para ello, se adoptó un
enfoque basado en contenedores visuales personalizados, cuyo tamaño se
define en términos relativos al espacio disponible en pantalla.

La figura \ref{fig:contenedores} ilustra este criterio. Cada contenedor
puede ocupar un porcentaje del ancho y del alto de la pantalla, lo que
permite estructurar la interfaz mediante una disposición en filas y
columnas.

\begin{figure}[h]
	\centering
	\includegraphics[scale=0.66]{./Figures/contenedores_ionic.png}
	\caption{Contenedores dentro de una pantalla.}
	\label{fig:contenedores}
\end{figure}

A partir de este esquema, las pantallas se construyen como una
composición de contenedores anidados. Dentro cada bloque final se integra cada uno de los elementos
funcionales de la aplicación, tales como indicadores, controles y
visualizaciones de datos.

Este enfoque responde a un modelo de layout basado en grillas,
ampliamente utilizado en el desarrollo de interfaces modernas. Su uso
permitió mantener una distribución consistente de los elementos en
todas las pantallas y facilitó la adaptación del diseño a distintos
tamaños de dispositivo.

En el ejemplo de la figura \ref{fig:contenedores}, se observa un contenedor principal que
define el área de trabajo de la pantalla. A partir de este, se
establecen subdivisiones en filas y columnas mediante nuevos
contenedores, lo que permite organizar la ubicación de cada elemento de
la interfaz.
...
La pestaña principal, representada en la figura  \ref{fig:app_home}, constituye el punto de acceso central a la
información del sistema, ya que concentra las variables más relevantes
asociadas al estado general del motorhome.

\begin{figure}[h]
	\centering
	\includegraphics[scale=0.16]{./Figures/P1_Home.png}
	\caption{Pestaña principal de la aplicación.}
	\label{fig:app_home}
\end{figure}

Desde el punto de vista funcional, esta pantalla implementa un
mecanismo de actualización periódica basado en solicitudes HTTP de tipo
GET al endpoint home. 

Para ello, se utiliza un temporizador interno que permite consultar de forma cíclica el estado del sistema. De este modo, la información visualizada se mantiene sincronizada con las variables reales del motorhome.

La información presentada se organiza en distintos grupos funcionales. Por un lado, se incluyen variables asociadas al sistema de baterías, tales como tensión, corriente y porcentaje de carga. Por otro lado, se presentan magnitudes correspondientes al sistema de corriente alterna, entre las que se encuentran la tensión, la corriente y la frecuencia de entrada.

Adicionalmente, se incorporan indicadores que permiten identificar el
origen de la alimentación del sistema. Esta lógica responde al
comportamiento del módulo PRC10, que establece un esquema de prioridad
entre las distintas fuentes disponibles. 

En este esquema, la línea de
alimentación posee la mayor prioridad, seguida por el inversor y,
finalmente, el grupo electrógeno. La interfaz refleja este criterio de
forma visual, lo que permite interpretar rápidamente el estado de
operación del sistema.

Asimismo, la pantalla incluye la visualización del nivel de los tanques
de agua, representados en términos porcentuales, lo que permite un
monitoreo directo de estos recursos.

En cuanto al control del sistema, se incorporan pulsadores que permiten
accionar distintos actuadores del motorhome, como la bomba de agua y
las esclusas de los tanques. Estos comandos se envían mediante
solicitudes HTTP de tipo POST al endpoint pulsadores, donde se
transmite la identificación del control y su nuevo estado en formato
\texttt{JSON}.

De esta manera, la pestaña principal integra en una única interfaz la visualización de variables críticas y el acceso a funciones de control, y representa el principal punto de interacción entre el usuario y el sistema.

\subsection{Aplicación: pestaña luces}
\label{sec:app_luces}

La pestaña correspondiente al control de iluminación, representada en la figura \ref{fig:app_luces}, concentra las funciones asociadas al manejo de las luces del sistema.
...
Desde el punto de vista funcional, esta pantalla implementa un
mecanismo de actualización periódica mediante solicitudes HTTP de tipo
GET al endpoint luces. A través de estas consultas, se obtienen
los estados de los circuitos de iluminación y el valor asociado al
control de intensidad.

En particular, el sistema permite gestionar tanto la iluminación
interior como la exterior. Para el caso de la iluminación interior, se
dispone de un control de intensidad basado en modulación por ancho de
pulso (PWM) \citep{web_pwm}, implementado en el módulo PRC10. Esto
permite ajustar el nivel de iluminación mediante la variación del
ciclo de trabajo de la señal.

Las acciones de control se envían mediante solicitudes HTTP de tipo
POST al endpoint pulsadores, donde se transmite la
identificación del elemento a accionar y el valor correspondiente. Este
esquema unifica el mecanismo de control para distintos tipos de
actuadores, tanto discretos como analógicos.

De esta manera, la pestaña de iluminación permite supervisar el estado de los circuitos y ajustar su comportamiento a través de una interfaz simple, en coherencia con la arquitectura de comunicación definida para el sistema.
\subsection{Aplicación: pestaña motores}
\label{sec:app_motores}

La pestaña de motores, representada en la figura  \ref{fig:app_motores}, se encarga de la supervisión y el control de los
elementos móviles del motorhome.
...
Desde el punto de vista funcional, esta pantalla implementa un
mecanismo de actualización periódica mediante solicitudes HTTP de tipo
GET al endpoint motores. A través de estas consultas, se
obtienen tanto los estados de los actuadores como las variables
necesarias para la lógica de control.

La interfaz permite seleccionar el elemento móvil sobre el que se
desea operar y presentar los controles asociados a dicho elemento. De
esta forma, se reutiliza una misma estructura de control para distintos
actuadores, lo que simplifica el diseño de la aplicación.

Entre las variables recibidas se incluye el estado del freno de mano
del vehículo. Esta señal se utiliza como condición de habilitación para
determinados actuadores, en particular la cabina desplazable y el toldo.

Cuando el freno de mano no se encuentra accionado, el sistema bloquea
la interacción con estos controles. Esta restricción responde a
criterios de seguridad, ya que dichos mecanismos solo deben operar con
el vehículo detenido. Una vez que el sistema detecta la activación del
freno de mano, los controles correspondientes se habilitan.

Por otro lado, el motor asociado al desplazamiento de la mesa no
depende de esta condición, por lo que su control permanece habilitado
en todo momento.

Las acciones de control se envían mediante solicitudes HTTP de tipo
POST al endpoint pulsadores, donde se transmite la
identificación del actuador y el estado requerido.

De esta manera, la pestaña de motores centraliza el control de los
elementos móviles e incorpora restricciones de operación que
garantizan un uso seguro del sistema.

\subsection{Aplicación: pestaña energía}
\label{sec:app_energia}

La pestaña de energía, representada en la figura \ref{fig:app_energia}, concentra la supervisión y el control de los
principales sistemas energéticos del motorhome, entre ellos el grupo
electrógeno, el cargador, el inversor y otros consumos de alta
potencia.
...
Desde el punto de vista funcional, esta pantalla implementa un
mecanismo de actualización periódica mediante solicitudes HTTP de tipo
GET al endpoint energia. A través de estas consultas se
obtienen las variables asociadas al estado de los distintos equipos,
lo que permite su monitoreo en tiempo real.

La interfaz agrupa los controles según el tipo de sistema. En primer
lugar, se dispone del control del grupo electrógeno, que permite su
encendido y apagado, así como la selección del modo de operación del
cebador. 

Por otro lado, se incluyen los controles asociados al cargador
y al inversor, junto con la visualización de variables relevantes como
la corriente de carga y la potencia suministrada.

Además, se incorporan controles correspondientes a otros sistemas de
consumo energético, como el calentador de agua y el sistema de gas
envasado de alta potencia. Esta integración permite centralizar el
manejo de distintos dispositivos dentro de una misma interfaz.

Las acciones de control se envían mediante solicitudes HTTP de tipo
POST al endpoint pulsadores, donde se transmite la
identificación del equipo y el estado requerido.

De esta manera, la pestaña de energía permite supervisar y gestionar
los distintos subsistemas energéticos desde un único punto de acceso,
en coherencia con la arquitectura de control definida para el sistema.
