...
En este capítulo se explica cómo se diseñó el dispositivo, cómo está conformado el hardware y el software del equipo, y cómo se desarrolló la aplicación para el control del motorhome.

\section{Desarrollo de hardware}
\label{sec:desarrollo_hardware}

Como se mencionó anteriormente, el ESP32 debe actuar como intermediario y vínculo entre las tres partes principales: módulo PRC10, Cerbo GX y aplicación móvil.
...
el módulo transceiver (mostrado en la figura \ref{fig:placa_transceiver}) utilizado en conjunto con la pantalla HMI de control.
...
El equipo utilizado para los ensayos fue un Victron MultiPlus-II 48/15000/200-100 230V \citep{web_multiplus}, provisto por la empresa Enertik. Si bien el desarrollo
...
formato JSON (\textit{JavaScript Object Notation}) \citep{web_json} con la información requerida por la pantalla
...
implementó un quinto \textit{endpoint} (denominado pulsadores) que admite solicitudes de tipo
...
\textit{endpoints} definidos en el servidor del ESP32. Para ello, el
...
HTTP de tipo GET y POST hacia los \textit{endpoints}
correspondientes.
...
Para el desarrollo de la aplicación, se tomó como referencia la
aplicación utilizada en la pantalla HMI del motorhome. Se buscó
replicar la mayor cantidad posible de sus funcionalidades, con
prioridad en aquellas consideradas de mayor importancia.
...
\textit{home} y corresponde a la pantalla principal.

\begin{figure}[h]
	\centering
	\includegraphics[scale=0.16]{./Figures/P1_Home.png}
	\caption{Pestaña \textit{home} de la aplicación.}
	\label{fig:app_home}
\end{figure}
...
\textit{endpoint} \textit{home}. A través de este mecanismo se obtienen
...
realizan solicitudes de tipo POST al \textit{endpoint} denominado
pulsadores. En este caso, se envía un archivo JSON que indica
...
solicitudes GET de forma periódica al \textit{endpoint} denominado
luces.
...
realiza una solicitud POST al \textit{endpoint} pulsadores,
...
\textit{endpoint} denominado motores y se encarga del control
...
una imagen con el símbolo de precaución o \textit{warning}. Cuando el
...
Esta pestaña realiza solicitudes GET de forma periódica al
\textit{endpoint} denominado energía y se encarga del control
...
A partir de este proceso, el proyecto fue importado en el entorno de
